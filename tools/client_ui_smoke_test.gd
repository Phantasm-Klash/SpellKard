...
func _assert_target_page_contract(snapshot: Dictionary, label: String, layout_tokens: Array[String], status_tokens: Array[String], asset_usage_token: String) -> bool:
	...
	var focus_lane_text := String(snapshot.get("page_focus_lanes", ""))
	if not _contains_all(focus_lane_text, _expected_focus_lanes_for(label)):
		return _fail("%s focus lanes missing expected runtime lanes %s" % [label, focus_lane_text])
