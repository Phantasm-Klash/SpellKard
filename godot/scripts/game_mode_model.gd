...
func boss_formation_display_summary(mode_id: String, playfield: Rect2 = Rect2(Vector2.ZERO, Vector2.ONE)) -> Dictionary:
	...
	return {
		...
		"slot_summary_text": " | ".join(slot_summaries),
		...
		"hp_persistence_policy": "persistent_global_hp" if mode_id == MODE_WORLD_BOSS else "per_instance_hp",
...
func boss_playfield_projection(mode_id: String, playfield: Rect2 = Rect2(Vector2.ZERO, Vector2.ONE)) -> Dictionary:
	...
	var slot_points: Array = formation_display_summary.get("slot_points", [])
	var slot_summaries := _string_array(formation_display_summary.get("slot_summaries", []))
	...
	return {
		...
		"slot_display_points": slot_points,
		"slot_summary_text": " | ".join(slot_summaries),
		"formation_contract": contract,
		"roster_authority_contract": roster_authority,
		"roster_projection_scope": String(roster_authority.get("roster_projection_scope", "local_display_only")),
		...
		"center_aim_valid": bool(formation_display_summary.get("center_aim_valid", false)),
		"all_slots_face_center": bool(formation_display_summary.get("all_slots_face_center", false)),
		"player_count": int(formation.get("player_count", 0)),
		"slot_layout_policy": String(formation.get("slot_layout_policy", "")),
		"slot_labels": formation.get("slot_labels", []),
		"formation_valid": bool(formation.get("ok", false)),
...
func boss_hud_projection(mode_id: String, playfield: Rect2 = Rect2(Vector2.ZERO, Vector2.ONE)) -> Dictionary:
	...
	return {
		...
		"projection_scope": "local_display_only",
		"persistent_hp": mode_id == MODE_WORLD_BOSS,
		"hp_persistence_policy": String(playfield_projection.get("hp_persistence_policy", "persistent_global_hp" if mode_id == MODE_WORLD_BOSS else "per_instance_hp")),
		"boss_instance_id": String(state.get("boss_instance_id", "")),
...
func boss_local_status_row(row_id: String, mode_id: String) -> Dictionary:
	...
	return {
		...
		"slot_display_points": display_projection.get("slot_display_points", []),
		"slot_summary_text": String(display_projection.get("slot_summary_text", "")),
		"center_aim_valid": bool(display_projection.get("center_aim_valid", false)),
		"all_slots_face_center": bool(display_projection.get("all_slots_face_center", false)),
		"hp_persistence_policy": String(display_projection.get("hp_persistence_policy", "persistent_global_hp" if mode_id == MODE_WORLD_BOSS else "per_instance_hp")),
		"practice_preview": practice_preview,
		"practice_preview_scope": String(practice_preview.get("preview_authority_scope", BOSS_LOCAL_PREVIEW_AUTHORITY_SCOPE)),
...
func boss_display_contract_row(row_id: String, mode_id: String) -> Dictionary:
	...
	return {
		...
		"display_slots": playfield_projection.get("display_slots", []),
		"slot_display_points": playfield_projection.get("slot_display_points", []),
		"slot_summary_text": String(playfield_projection.get("slot_summary_text", "")),
...
func boss_display_health_projection(mode_id: String) -> Dictionary:
	...
	return {
		...
		"slot_display_points": playfield.get("slot_display_points", []),
		"slot_summary_text": String(playfield.get("slot_summary_text", "")),
		"center_aim_valid": bool(playfield.get("center_aim_valid", false)),
		"all_slots_face_center": bool(playfield.get("all_slots_face_center", false)),
...
func boss_practice_validation_projection(mode_id: String) -> Dictionary:
	...
	return {
		...
		"phase_validation_render_slot": "mode_cards",
		"phase_validation_summary_text": _boss_practice_phase_validation_summary_text(phase_cards),
		"formation_valid": bool(formation.get("ok", false)),
...
func _boss_practice_phase_validation_cards(phase_rows: Array[Dictionary]) -> Array[Dictionary]:
	var cards: Array[Dictionary] = []
	for raw_row in phase_rows:
		var phase_row: Dictionary = raw_row
		var phase_id := String(phase_row.get("phase_id", ""))
		var status := String(phase_row.get("validation_status", ""))
		cards.append({
			"id": "boss_practice_phase_card_%s" % phase_id,
			"phase_id": phase_id,
			"phase_index": int(phase_row.get("phase_index", cards.size())),
			"phase_validation_card_kind": "boss_practice_phase_validation_summary",
			"overview_card_kind": "boss_practice_phase_validation",
			"render_slot": "mode_cards",
			"validation_status": status,
			"enabled": status == "ready",
			"primary_metric": "digest %d" % int(phase_row.get("preview_signature_digest", 0)),
			"secondary_metric": "samples %d max_emit %d headroom %d" % [
				int(phase_row.get("sample_count", 0)),
				int(phase_row.get("max_emit_per_tick", 0)),
				int(phase_row.get("budget_headroom", 0)),
			],
			"sample_count": int(phase_row.get("sample_count", 0)),
			"sample_window_start_tick": int(phase_row.get("sample_window_start_tick", 0)),
			"sample_window_end_tick": int(phase_row.get("sample_window_end_tick", 0)),
			"sample_window_stride_ticks": int(phase_row.get("sample_window_stride_ticks", 0)),
			"max_emit_per_tick": int(phase_row.get("max_emit_per_tick", 0)),
			"budget_headroom": int(phase_row.get("budget_headroom", 0)),
			"preview_signature_digest": int(phase_row.get("preview_signature_digest", 0)),
			"expected_signature_digest": int(phase_row.get("expected_signature_digest", 0)),
			"performance_budget_status": String(phase_row.get("performance_budget_status", "")),
			"preview_authority_scope": String(phase_row.get("preview_authority_scope", BOSS_LOCAL_PREVIEW_AUTHORITY_SCOPE)),
			"replay_verification_scope": "local_practice_hash",
			"local_hash_authority": "local_practice_verification_only",
			"online_replay_authority": "server_audit_required",
			"damage_authority": "server",
...
func _boss_practice_phase_validation_summary_text(phase_cards: Array[Dictionary]) -> String:
	var ready_count := 0
	var blocked_count := 0
	var max_emit := 0
	var min_headroom := 0
	for card in phase_cards:
		if String(card.get("validation_status", "")) == "ready":
			ready_count += 1
		else:
			blocked_count += 1
		max_emit = maxi(max_emit, int(card.get("max_emit_per_tick", 0)))
		var headroom := int(card.get("budget_headroom", 0))
		if min_headroom == 0 or headroom < min_headroom:
			min_headroom = headroom
	return "phases ready %d blocked %d max_emit %d min_headroom %d" % [ready_count, blocked_count, max_emit, min_headroom]
...
func _boss_formation_row(row_id: String, mode_id: String, state: Dictionary) -> Dictionary:
	...
	return {
		...
		"formation_display_summary": formation_display_summary,
		"formation_display_signature": int(formation_display_summary.get("formation_display_signature", 0)),
		"slot_display_points": formation_display_summary.get("slot_points", []),
		"slot_summary_text": String(formation_display_summary.get("slot_summary_text", "")),
		"center_aim_valid": bool(formation_display_summary.get("center_aim_valid", false)),
		"all_slots_face_center": bool(formation_display_summary.get("all_slots_face_center", false)),
		"player_count": int(validation.get("player_count", 0)),
		"min_players": BOSS_MIN_PLAYERS,
...
func _boss_practice_validation_row(row_id: String, mode_id: String) -> Dictionary:
	...
	return {
		...
		"ui_control_label_key": "ui.control_card",
		"ui_action": "start_boss_practice_preview",
		"validation_card_action_hint": "start local practice preview from validation; Replay hash only",
		"local_practice_action": "start_boss_spellbook_run",
		"local_practice_target_screen": "practice",
		...
		"phase_validation_render_slot": String(projection.get("phase_validation_render_slot", "mode_cards")),
		"phase_validation_summary_text": String(projection.get("phase_validation_summary_text", "")),
		"formation_valid": bool(projection.get("formation_valid", false)),
		"center_aim_valid": bool(projection.get("center_aim_valid", false)),
		"all_slots_face_center": bool(projection.get("all_slots_face_center", false)),
		"player_count": player_count,
